...
$error = $bookModel->setupBook("update");
  
?>

<form method="post" class="w-1/2 mx-auto bg-white border rounded p-4 shadow-md">
  <h1 class="text-6xl font-light text-center mb-10 uppercase">Modifier un livre</h1>
  ...
  <div class="mb-4">
    ...
    <input value="<?= $book->getTitle() ?>" class="border rounded border-gray-100 py-2 px-4 w-full outline-none shadow-sm" type="text" name="title" id="title">
  </div>

  <div class="mb-4">
    <label class="block mb-2" for="author">Auteur du livre:</label>
    <input value="<?= $book->getAuthor() ?>" class="border rounded border-gray-100 py-2 px-4 w-full outline-none shadow-sm" type="text" name="author" id="author">
  </div>

  <div class="mb-4">
    <label class="block mb-2" for="image">Image du livre:</label>
    <input value="<?= $book->getImage() ?>" class="border rounded border-gray-100 py-2 px-4 w-full outline-none shadow-sm" type="text" name="image" id="image">
  </div>

  <div class="mb-4">
    <label class="block mb-2" for="price">Prix du livre:</label>
    <input value="<?= $book->getPrice() ?>" class="border rounded border-gray-100 py-2 px-4 w-full outline-none shadow-sm" type="number" step="0.01" name="price" id="price">
  </div>
